...
Write general exercise results inside `output/`. Host exercises may create personal configuration, generated .cbl/JCL, reviews and reports directly in `host-lab/`, and downloaded records under `host-lab/logs/`. Preserve supplied `host-lab` Markdown instructions, fixtures and templates. Do not modify supplied `z-lab/` sources; place newly generated documentation under `output/`.
...
Do not modify `z-lab/`, `tests/`, `scripts/`, `samples/`, `specs/`, `prompts/`, or `reference/` to make checks pass.
