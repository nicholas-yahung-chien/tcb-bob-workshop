...
For the optional real IMS exercise, follow host-lab/IMS-LAB.md and create personal JCL, review and report under host-lab/ims/. Preserve z-tests/ims-run.jcl. Its inline DBD, PSB and IMSCKP helper are a teaching model using job-private databases, not the customer's existing IMS schema. CKP02 still runs against a sequential file. Distinguish application DL/I diagnostics, IMS system messages and actual binary-log utility output. Never substitute samples/logs/ims-events.csv for real run evidence.
