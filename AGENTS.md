...
Write general exercise results inside `output/`. Host exercises may create personal configuration, generated .cbl/JCL, reviews and reports directly in `host-lab/`, and downloaded records under `host-lab/logs/`. Preserve supplied `host-lab` Markdown instructions and `z-tests/` test programs, fixtures and JCL template. Do not modify supplied `z-lab/` sources. Save course-authored documents under `output/`, preserve workflow outputs under `docs/`, and save host reviews and reports under `host-lab/`.
