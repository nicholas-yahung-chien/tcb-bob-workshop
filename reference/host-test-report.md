...
原版 CCKP 可能 RC 4，其餘 14 步預期 RC 0。需讀完整 CCKP/SYSPRINT 的全部診斷，核對 IGYLI0090-W、IGYSC0205-W、IGYCB7310-W 及實際原文。序號檢查與控制流程等原因應對回程式或選項；IGYSC0205-W 是摘要提醒，不能當成獨立根因。報告中填入本次清單出現的訊息。
...
中文 PRINTDD 依課程以 tcb-rse／IBM-1371 開啟、載入完整內容並另存 UTF-8；全部下載若已錯誤轉碼，不能只換本機開啟編碼。中文畫面欄數不是 byte 位置。若 job 缺少 SNAP 或 BEFORE-ID，記下無法比對的部分。

建議結論寫法：「本次已取得的紀錄支持……；尚缺……，因此……仍待確認。」完成前請核對 15 個步驟與各項比對結果。這項作業不是 IMS 交易，也不是新增預覽或 CKP02H 的驗證。
