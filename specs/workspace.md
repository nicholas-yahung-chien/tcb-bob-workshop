...
第 04 單元需確認 host-lab/IMS-LAB.md、host-lab/IMS-EVENTS.md、z-tests/ims-run.jcl、z-tests/IMSCKP.cbl、z-tests/imsevts.jcl 與 z-tests/IMSEVT.cbl。RC 4 修正為選做，另依 host-lab/RC4-LAB.md 操作。IMS 範本內含教學用 DBD 與 PSB，CIMS 讀取已備妥的遠端 COBOL(IMSCKP) 獨立來源，同一份作業已備妥於自己的 TCBLAB.JCL(IMSRUN)，不需學員重新產生或上傳。核對結果與報告放在 host-lab/ims；只有自行修改時才另存個人 JCL。它不取代基準 run.jcl，也不是客戶正式 IMS 環境的配置。

確認 `.bob/skills/cobol-explain-zh-tw/SKILL.md`、`.bob/skills/data-dictionary-zh-tw/SKILL.md` 與 `.bob/skills/program-docs-zh-tw/SKILL.md` 三份檔案齊全；保留 `.bob/rules/language.md`。Explain code 使用 cobol-explain-zh-tw；第 01 單元另用 data-dictionary-zh-tw，將實際產生的 DD.json 與來源整理成可閱讀資料字典。請學員在 Bob Settings → Skills 核對這兩項主流程技能均為 Workspace，且 Active 已開啟，將檔案檢查與實際畫面確認分別記錄在 output/workspace-check.md。未查看畫面時，技能啟用狀態保留待確認。Generate program documentation 已在課程環境確認能直接產生中文正文，直接使用產生的文件，不要求再呼叫翻譯技能。program-docs-zh-tw 只在學員需要翻譯英文正文時作為備援。檔案存在不等於已確認技能啟用，不要在準備階段自動執行工作流程。
