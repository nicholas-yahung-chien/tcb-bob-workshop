...
選做第 04 單元延伸練習時，另確認 host-lab/RC4-LAB.md、host-lab/IMS-LAB.md 與 z-tests/ims-run.jcl。IMS 範本內含本次練習的教學用 DBD、PSB 與輔助程式，同一份作業已備妥於自己的 TCBLAB.JCL(IMSRUN)，不需學員重新產生或上傳。核對結果與報告放在 host-lab/ims；只有自行修改時才另存個人 JCL。它不取代基準 run.jcl，也不是客戶正式 IMS 環境的配置。
