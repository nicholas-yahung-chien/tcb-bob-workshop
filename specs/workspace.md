...
後續課程加入檔案時，使用 z-lab 下的 .cbl、.cpy 與 .asm。CKP02 使用 z-lab/CKP02.cbl；CIS14 使用 z-lab/CIS14/CIS14.cbl 與同目錄的 .cpy。教材已直接提供這些檔案，不需要從 TXT 轉換或建立副本；使用根目錄 source-manifest.json 核對完整性。主機 JCL 內嵌的編譯副本則依 host-lab 指引準備。
