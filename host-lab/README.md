...
CASE 0001、0002 為一般格式模擬；CASE 0003–0007 分別保留 EOF 排除、重跑、鄰近位置 EOF、非數字識別值與空白識別值的回歸測試。CASE 0003 放在中間是刻意確認既有程式略過轉換後仍繼續讀取，不代表正式尾筆檔案配置。識別值用來測試轉換條件，未檢查統編或身分證有效性。正式業務規則仍需依客戶的資料字典確認。
