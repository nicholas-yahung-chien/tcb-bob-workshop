...
欄位位置以 CKP02 的有效宣告為準，全部為 DISPLAY 欄位，共 400 bytes。fixtures.json 的 fields 逐欄列出內容，字串右側補空白；不再填入 Q 或在 FILLER 放案例編號。使用英文虛構名稱及地址，確保一個字元對應一個 byte，方便直接核對位置；這組測資不涵蓋中文 DBCS 資料案例。
...
| R-NOTE | 250–309 | 明示 SYNTHETIC，表示課程模擬資料 |
| FILLER | 310–400 | 全部空白 |
