...
先用 PPZ 掃描 CKP02，了解程式結構，再建立 Data Dictionary（資料字典），整理各欄位的用途。這些資訊會用在後續的需求分析與程式文件中。

## 1. 掃描 CKP02

1. 開啟 z-lab/CKP02.cbl，切換 Z Architect，將程式加入對話。
...
掃描與第 05 單元的 ZCodeScan 用途不同：前者提供程式結構，後者檢查程式碼規則。
