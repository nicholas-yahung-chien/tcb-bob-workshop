...
| 設計提案 | 比較做法與限制，不把建議当成客戶承諾；規則不變不等於程式完全不能修改 |
| 本次執行證據 | 必須有相同 job ID 的實際紀錄；尚未取得時標示待執行或待取證，不把已知環境改寫成未知 |

課程來源使用 IBM-937，三支程式的編譯均使用 CODEPAGE(937),DBCS；JCL 與一般紀錄使用 IBM-1047。每個帳號已備妥 PDSE、FB80 的 <帳號>.TCBLAB.COBOL，內含與教材同名的 CKP02、GENCKP、CHKCKP 成員，以及 <帳號>.TCBLAB.JCL(RUN)。CKP02 對應 z-lab/CKP02.cbl，兩支測試輔助程式對應 z-tests 中的同名檔案。這些是既有基準，不用新程式覆寫。若提案新增程式，可另規劃新成員及 JCL 整合；不要將新增程式當成已提供或已驗證。
