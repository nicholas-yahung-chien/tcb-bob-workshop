...
完成正文後，回頭用這份記錄核對開頭摘要、表格與例子。如果同一件事有不同說法，先查原始碼再修正，讓摘要與詳細說明保持一致。
...
核對結果區分「文件敘述有依據」「需求仍待確認」「設計尚未完整」「已完成執行驗證」。開始實作前，再確認影響設計的問題已有答案。
...
每筆 evidence 包含 file、line、quote。file 使用 z-lab/CKP02.cbl；line 從 1 起算，quote 取該行實際原文，不改寫。引用前核對程式與 source-manifest.json；若學員已修改程式，先說明版本及修改差異。
