請閱讀 AGENTS.md、SOURCE-MAP.md、specs/change-request.md、specs/customer-context.md、bank-source/reading/CKP02.TXT、samples/cobol/IDSHIFT.cbl、samples/cobol/IDREC.cpy，及前一步可用的CR-01規劃。
...
只寫上述兩個檔案，不執行業務程式，不實作需求。完成後摘要兩份產出與待確認事項。
...
marker_field只放欄位名稱R-NAME；正文說明放JSON之外。請使用原文檔案編輯工具保存Markdown。建立後執行python scripts/verify.py plan，只修正output中的失敗。
