...
for file in sorted(source.rglob('*.TXT')):
    ...
    suffix = '.asm' if asm else '.cbl' if file.stem in ('CKP02', 'CIS14') else '.cpy'
    learner_path = Path('z-lab') / file.relative_to(source).with_suffix(suffix)
    index.append({'file':learner_path.as_posix(),'lines':len(lines),
