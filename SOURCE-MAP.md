...
主要分析素材是 `bank-source/reading/` 的合庫原始程式閱讀版；使用者已確認合庫同意公開這批原始碼。`bank-source/raw/` 保留45份TXT原始位元組，`manifest.json`記錄SHA-256、編碼與行數。SYSOCP31含無法以CP950解碼的bytes，閱讀版明示為反斜線x序列，不猜測其字義。閱讀版可供AI分析，不能替代主機編譯來源。
...
| samples/reports/sast.json | 教學者編寫的 finding，非 Fortify/Checkmarx 真實掃描結果 |
| specs/change-request.md | 本次假設的新需求，不代表合庫已核定需求 |
| specs/customer-context.md | 合庫提供素材的人工摘要與缺件邊界 |

原始工作單畫面未納入公開庫，使用空白欄位模板代替。未提供真實個資測試資料，所有測試值均由教學者合成。
