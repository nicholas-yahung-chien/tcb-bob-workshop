...
教材已在 z-lab 提供 45 份程式與相依檔案，使用 .cbl、.cpy、.asm 副檔名。source-manifest.json 記錄每份檔案的 SHA-256，供完整性核對。教材檔案使用 UTF-8 與 LF 換行；封裝時僅統一換行，不翻譯註解或改寫程式。CKP02 主機練習直接上傳同一份 z-lab/CKP02.cbl，以 IBM-937 傳輸並編譯。
...
| requests/ | 業務需求信件（EML）與 Word 申請單；同一項尚待釐清的需求 |
