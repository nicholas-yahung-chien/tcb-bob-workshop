...
程式、JCL 與作業紀錄統一透過 IBM RSE API 連線。開啟 COBOL 選 `tcb-rse`（IBM-1371）；開啟 JCL、提交作業與查看 JES 系統紀錄選 `tcb-rse`（IBM-1371）；CHECK1／CHECK2 輸出也選 `tcb-rse`；含中文的編譯清單選 `tcb-rse`（IBM-1371）。IBM Z Open Editor 與 Zowe Explorer 需保持啟用。
